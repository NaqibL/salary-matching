...
export async function GET(request: NextRequest) {
  ...
  const limit = Math.min(Math.max(parseInt(request.nextUrl.searchParams.get('limit') || '30', 10), 1), 50)
  try {
    const data = await unstable_cache(
      async () => {
        const res = await fetch(
          `${API_BASE_URL}/api/dashboard/jobs-by-category-public?limit_days=${limitDays}&limit=${limit}`,
          { headers: { 'Content-Type': 'application/json' }, next: { revalidate: REVALIDATE_SECONDS } }
        )
        if (!res.ok) throw new Error(`Backend error: ${res.status}`)
        return res.json()
      },
      ['dashboard-jobs-by-category', String(limitDays), String(limit)],
    ...
    console.error('[dashboard/jobs-by-category]', err)
