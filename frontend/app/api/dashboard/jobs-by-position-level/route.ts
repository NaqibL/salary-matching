...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await unstable_cache(
      async () => {
        const res = await fetch(
          `${API_BASE_URL}/api/dashboard/jobs-by-position-level-public?limit_days=${limitDays}&limit=${limit}`,
          { headers: { 'Content-Type': 'application/json' }, next: { revalidate: REVALIDATE_SECONDS } }
        )
        if (!res.ok) throw new Error(`Backend error: ${res.status}`)
        return res.json()
      },
      ['dashboard-jobs-by-position-level', String(limitDays), String(limit)],
    ...
    console.error('[dashboard/jobs-by-position-level]', err)
